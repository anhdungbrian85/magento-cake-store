...
class ProcessSourceItemsObserver implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        // var_dump($productData);echo "</br>xxxxxxxxxxxxxxxxxxxxx</br>";
        $singleSourceData = $productData['quantity_and_stock_status'] ?? [];
        // var_dump($singleSourceData);echo "</br>xxxxxxxxxxxxxxxxxxxxx</br>";
        ...
        if (!$this->isSingleSourceMode->execute()) {
            ...
            $assignedSources =
                ...
                && is_array($sources['assigned_sources'])
                    ...
                    : [];
            $unassignedSources =
                isset($sources['unassigned_sources'])
                && is_array($sources['unassigned_sources'])
                    ? $this->prepareAssignedSources($sources['unassigned_sources'])
                    : [];
            $sourceData = array_merge($assignedSources, $unassignedSources);
            $this->sourceItemsProcessor->execute((string)$productData['sku'], $sourceData);
    ...
    private function prepareAssignedSources(array $assignedSources): array
    {
        foreach ($assignedSources as $key => $source) {
            ...
            if (!key_exists('initialize', $source)) {
                $source['initialize'] = 'true';
                $assignedSources[$key] = $source;
            }
            if (!key_exists('record_id', $source)) {
                $source['record_id'] = $source['source_code'];
                $assignedSources[$key] = $source;
            }
        }
