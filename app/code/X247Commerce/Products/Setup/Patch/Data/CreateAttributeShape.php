...
class CreateAttributeShape implements DataPatchInterface
{
    ...
    public function apply()
    {
        ...
        if( !$eavSetup->getAttributeId(\Magento\Catalog\Model\Product::ENTITY, 'shape')) {
            $eavSetup->removeAttribute(\Magento\Catalog\Model\Product::ENTITY, 'shape');
        }

        $eavSetup->addAttribute(\Magento\Catalog\Model\Product::ENTITY, 'shape', [
           'type' => 'int',
           'label' => 'Shape',
           'input' => 'select',
    ...
    public function convertAttributeToSwatches() {
        $attribute = $this->eavConfig->getAttribute('catalog_product', 'shape');
        if (!$attribute) {
            return;
        }
